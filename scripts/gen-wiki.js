...
function genHome() {
  let content = fs.readFileSync(path.join(pathTemplates, "Home.md"), "utf-8");
  return content;
}

function genSidebar(catalog) {
  let content = fs.readFileSync(path.join(pathTemplates, "_Sidebar.md"), "utf-8");
  let makerlist = [];
  for (const a of catalog) {
...
function genFooter() {
  let content = fs.readFileSync(path.join(pathTemplates, "_Footer.md"), "utf-8");
  return content;
}
